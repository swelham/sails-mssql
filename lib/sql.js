...
function wrapSqlParameterValue (attr, options) {console.dir(options);
...
function wrapSqlUpdateStatement (attr, options) {
	return util.format('%s=%s', wrapSqlObjectName(attr, options), wrapSqlParameter(attr, options));
}
...
function getOptionsFromData (collection, data) {
	return {
...
var builders = {
	...
	update: function (options) {
		...
		query += util.format('UPDATE [%s] SET %s',
			options.table,
			build(options, wrapSqlUpdateStatement));
		// add on where statement
...
var sql = {
	insertQuery: function (collection, data) {
		return buildStatement('insert', getOptionsFromData(collection, data));
	},

	updateQuery: function (collection, data) {
		return buildStatement('update',  getOptionsFromData(collection, data));
	}
};
